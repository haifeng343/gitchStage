...
<script>
    import {getAgeStatistic} from 'api/userdata.js'
    ...
    export default {
        ...
        methods: {
            ...
            async getData() {
              ...
              await getAgeStatistic(params).then(res =>{
                if(res.Success){
                  let ageStatistic = res.Data.AgeStatistic_Sms
                  let arr = [] 
                  let nameArr = []
                  for(let i =0;i<ageStatistic.length;i++){
                    arr.push({name:ageStatistic[i].Name,value:ageStatistic[i].Count})
                    nameArr.push(ageStatistic[i].Name)
                  }
