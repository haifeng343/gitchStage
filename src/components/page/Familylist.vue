...
import { getFamilyList, setFamilyStatus, addFamilyList } from "api/userdata.js";
export default {
  name: "Familylist",
  data() {
    return {
  ...
  methods: {
    ...
    jumpUrl(item) {
      ...
      this.$router.push(`/familyDetail/${item.FamilyId}`);
    },
    // 修改switch的状态
    changeStatus(status, id) {
      ...
      setFamilyStatus(params).then(res => {
        if (res.Success) {
          this.$message({
            showClose: true,
            message: "更新成功",
