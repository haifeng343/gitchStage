<template>
  <div class="table">
    <div class="container">
      <el-form :inline="true" class="demo-form-inline">
        <el-form-item label="电销姓名:">
          <el-input v-model="accountName" placeholder="请输入电销姓名"></el-input>
        </el-form-item>
      ...
      <el-table :data="tableData" fixed border ref="multipleTable">
        ...
        <el-table-column align="center" label="话费余额">
          <template slot-scope="scope">
            <el-button type="text" @click="setBalance(scope.row)">{{scope.row.AdvanceBalance}}</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="SeatNumber" align="center" label="席位"></el-table-column>
        ...
        <el-table-column label="操作" width="250" align="center" fixed="right">
          <template slot-scope="scope">
            ...
            >设置主管</el-button>
            <el-button v-else type="text" @click="jumpToSubordinate(scope.row)">下属列表</el-button>
            <el-button type="text" @click="openChangePasswordForm(scope.row)">修改密码</el-button>
            <el-button type="text" @click="jumpToMoneychangelog(scope.row)">余额变更日志</el-button>
    ...
    <!-- 新增弹出框 -->
    <el-dialog title="创建电销人员" :visible.sync="addVisible" width="663px">
      <el-form ref="addForm" :model="addForm" :rules="addrules" label-width="100px">
        <el-form-item label="电销姓名:" prop="AccountName">
          <el-input v-model="addForm.AccountName" placeholder="请输入电销人员姓名"></el-input>
        </el-form-item>
        <el-form-item label="手机号" prop="AccountMobile">
          <el-input v-model="addForm.AccountMobile" placeholder="请输入电销人员手机号"></el-input>
        </el-form-item>
        ...
        <el-form-item label="职位" prop="Level">
          <el-radio v-model="addForm.Level" label="1">电销人员</el-radio>
          <el-radio v-model="addForm.Level" label="2">主管</el-radio>
        </el-form-item>
      </el-form>
    ...
    <!-- 编辑弹出框 -->
    <el-dialog :title="`编辑用户-${editForm.AccountName}`" :visible.sync="editVisible" width="663px">
      <el-form ref="editForm" :model="editForm" :rules="editrules" label-width="100px">
        ...
        <el-form-item label="职位:" prop="Level">
          <el-radio v-model="editForm.Level" label="1">电销人员</el-radio>
          <el-radio v-model="editForm.Level" label="2">主管</el-radio>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    jumpToRoyaltyRule(item) {
      this.$router.push({
        path: `/delegateroyaltyrule/${item.AccountId}`,
        query: {
    ...
    jumpToSubordinate(item) {
      this.$router.push({
        path: `/delegatesubordinate/${item.AccountId}`,
        query: {
          name: item.AccountName
        }
      });
    ...
    //跳转到余额变更页面
    jumpToMoneychangelog(item) {
      this.$router.push({
        path: `/delegatemoneychangelog/${item.AccountId}`,
        query: {
          name: item.AccountName
        }
      });
